...
const production3: any = []
const result3 = manufactureGifts(production3)
console.log(result3)
// []
}

const manufactureGifts = (giftsToReduce: Array<{ toy: string, quantity: number }>): string[] => {
  const result = giftsToReduce.filter( gift => gift.quantity > 0 ).map( gift =>  gift.quantity > 0 ? Array(gift.quantity).fill(gift.toy) : []);

  return [...result.flatMap(gift => gift)];
}