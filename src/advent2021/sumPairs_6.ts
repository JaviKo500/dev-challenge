...
Una vez que tengas el resultado... ¿cómo podrías hacer que fuese lo más óptimo posible para no tener que recorrer 
las mismas situaciones dos veces 🤔?*/
export const sumPairs6 = () => {
  console.log('<--------------- JK SumPairs_6 --------------->');
  ...
  console.log(sumPairs([-1, 0, 3, 1, 4], 0)); // [-1, 1]
}

const sumPairs = (numbers: number[], result: number): number[] | null => {
  for ( let i = 0; i < numbers.length; i++) {
    const number = numbers[i] ?? 0;
    for (const num of numbers.slice(i + 1, numbers.length)) {
      const totalSum = number + num;
      if (totalSum === result) {
        return [number, num];
      }
    }
